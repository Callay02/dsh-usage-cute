...
/** Default SiliconFlow connection facts when no provider profile sets one. */
...
/** All top-level credential refs from the credentials file (names only). */
async function allCredentialRefs(ctx) {
	const refs = new Set();
	try {
		...
		for (const line of text.split("\n")) {
			...
			if (match !== null) refs.add(match[1]);
		}
	} catch {
		// missing/unreadable credentials file — no accounts
	}
	return [...refs].sort();
}

/** Balance scheme + base URL for a credential ref, inferred from its name. */
function accountSchemeOf(ref, ctx) {
	if (ref.startsWith(TOKEN_PREFIX)) {
		return { scheme: "deepseek", baseURL: deepseekProvider(ctx).baseURL };
	}
	if (ref.includes("SILICONFLOW") || ref === "OPENAI_API_KEY") {
		return { scheme: "siliconflow", baseURL: siliconflowBaseURL(ctx, ref) };
	}
	return { scheme: null, baseURL: "" };
}

/** Prefer a pi-ai provider profile that names this ref; fall back to default. */
function siliconflowBaseURL(ctx, ref) {
	const settings = ctx.get("settings");
	const pi = settings?.get?.("llm-pi-ai");
	if (pi !== void 0 && pi !== null && typeof pi === "object" && pi.providers !== void 0 && typeof pi.providers === "object") {
		for (const profile of Object.values(pi.providers)) {
			if (profile !== null && typeof profile === "object" && profile.apiKeyEnv === ref && typeof profile.baseURL === "string" && profile.baseURL.length > 0) {
				return profile.baseURL;
			}
		}
	}
	return SILICONFLOW_DEFAULTS.baseURL;
}

function accountDisplayName(ref, scheme, defaultRef) {
	if (scheme === "deepseek") return ref === defaultRef ? "DeepSeek（默认）" : `DeepSeek · ${ref}`;
	if (scheme === "siliconflow") return `硅基流动 · ${ref}`;
	return `${ref}（无余额接口）`;
}

/**
 * List switchable balance accounts: every credential ref, with the balance
 * scheme inferred from the key name (`DEEPSEEK_API_KEY*` → DeepSeek, keys
 * containing `SILICONFLOW` / `OPENAI_API_KEY` → SiliconFlow). Values never
 * leave the server — only ids and display names are returned.
 */
async function listAccounts(ctx) {
	const defaultRef = deepseekProvider(ctx).apiKeyEnv;
	const accounts = [];
	for (const ref of await allCredentialRefs(ctx)) {
		const { scheme, baseURL } = accountSchemeOf(ref, ctx);
		accounts.push({
			id: `${scheme ?? "other"}:${ref}`,
			provider: scheme ?? "other",
			displayName: accountDisplayName(ref, scheme, defaultRef),
			ref,
			baseURL,
			scheme,
	...
	accounts.sort((a, b) => (a.isDefault ? -1 : b.isDefault ? 1 : a.displayName < b.displayName ? -1 : 1));
	return accounts;
}

/** Resolve one account id (`<scheme>:<ref>`) to its full descriptor. */
