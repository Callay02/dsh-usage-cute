...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TOKENS_URL = "/api/usage-cute/tokens";
		const USAGE_URL = "/api/usage-cute/usage";
		const LOGO_URL = "/api/usage-cute/logo.png";
		const TOKEN_STORAGE_KEY = "dsh-usage-cute:token";
		...
		function UsageCutePanel({ wide, t }) {
			...
			const balanceUrl = selectedToken && selectedToken.length > 0
				? BALANCE_URL + "?token=" + encodeURIComponent(selectedToken)
				: BALANCE_URL;
			...
			//#region token switcher
			react.useEffect(() => {
				let cancelled = false;
				fetch(TOKENS_URL, { headers: { accept: "application/json" } })
					.then((response) => (response.ok ? response.json() : Promise.reject(new Error("HTTP " + response.status))))
					.then((data) => {
						if (!cancelled && Array.isArray(data?.tokens)) setTokens(data.tokens);
					})
					.catch(() => {
						// token list unavailable — default key only
					});
				return () => {
					cancelled = true;
				};
			...
			react.useEffect(() => {
				...
				if (tokens.some((tok) => tok.ref === selectedToken)) return;
				const fallback = tokens.find((tok) => tok.isDefault) ?? tokens[0];
				setSelectedToken(fallback?.ref ?? "");
			}, [tokens, selectedToken]);

			react.useEffect(() => {
				try {
					localStorage.setItem(TOKEN_STORAGE_KEY, selectedToken);
				} catch {
					// ignore
				}
			}, [selectedToken]);
			...
			const tokenPicker = tokens.length > 1
				? react.createElement("div", { className: "uc_tokenPicker" },
					react.createElement("span", { className: "uc_tokenPickerLabel" }, "🔑 " + t("balance.token")),
					react.createElement("select", {
						className: "uc_tokenSelect",
						value: selectedToken,
						onChange: (e) => setSelectedToken(e.target.value),
						title: t("balance.tokenHint")
					},
						tokens.map((tok) =>
							react.createElement("option", { key: tok.ref, value: tok.ref },
								tok.ref + (tok.isDefault ? "（默认）" : "")
							)
						)
					)
				)
		...
		const zh = {
			...
			"balance.token": "账号 Token",
			"balance.tokenHint": "切换查看不同 DeepSeek 账号的余额",
		...
		const en = {
			...
			"balance.token": "Account token",
			"balance.tokenHint": "Switch the DeepSeek account shown",
